...
Sample_or_Event <- df %>% select(substrate, depth) # We were not able to decide whether these two columns fell under Sample or Event. 
...
Sample <- df %>% select("basisOfRecord??" = sample_type_EMP, # Need to finalize if this will be the basis of record.
                          ...
                          "taxonomyPhyla AND taxonomyClass AND taxonomyOrder AND taxonomyClade AND genus AND specificEpithet" = full_taxonomy_string_with_clade, # Should the phyla, class, order, etc. and all just be split into their own separate columns, and remove the concatenated versions?
                          "taxonomyPhyla AND taxonomyClass AND taxonomyOrder AND taxonomyClade" = taxonomy_string_to_clade,
                          "taxonomyPhyla AND taxonomyClass AND taxonomyOrder AND taxonomyFamily" = taxonomy_string_to_family,
                          "taxonomyPhyla AND taxonomyClass AND taxonomyOrder" = taxonomy_string_to_order,
                          "taxonomyPhyla AND taxonomyClass AND taxonomySubclass" = taxonomy_string_to_subclass,
                          "taxonomyPhyla AND taxonomyClass" = taxonomy_string_to_class,
                          family = family,
                          ...
                          separate(physical_sample_location, c("principalInvestigator",
                                                               "institutionCode"), sep=",") # Separated physical sample location into principal investigator and institution code, removed institution country.




Sample_or_Event <- Sample_or_Event %>% rename(substratum = substrate, 
                                              "maximumDepthInMeters OR minimumDepthInMeters" = depth) # minimum or maximum?
